...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  try {
      ...
      .prepare(
        `SELECT
           ...
           (SELECT content FROM messages
            WHERE client_id = m.client_id AND session_id = m.session_id
         ...
         FROM messages m
         LEFT JOIN leads l
           ON l.client_id = m.client_id AND l.session_id = m.session_id
         WHERE m.client_id = ?
    ...
    console.error("[albamed/sessions GET] Error:", err)
