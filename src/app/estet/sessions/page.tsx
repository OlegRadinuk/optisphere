    ...
    <div>
      <style>{`
        ...
        @media (max-width: 767px) {
          ...
          .es-sess-card { background:#fff; border:1px solid #E2E8F0; border-radius:12px; padding:14px 16px; cursor:pointer; -webkit-tap-highlight-color:transparent; box-shadow:0 1px 2px rgba(15,23,42,0.04), 0 4px 12px rgba(15,23,42,0.05); }
          .es-sess-card:active { background:#F1F5F9; }
          ...
          .es-sess-msg { font-size:14px; color:#0F172A; overflow:hidden; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; margin-bottom:10px; line-height:1.45; }
          ...
          .es-sess-skel-card { background:#fff; border:1px solid #E2E8F0; border-radius:12px; padding:14px 16px; }
          .es-sess-skel-row { display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:8px; }
          .es-sess-skel-line { height:14px; background:#E2E8F0; border-radius:4px; animation:pulse 1.5s ease infinite; }
          .es-sess-skel-line-tall { height:36px; background:#E2E8F0; border-radius:4px; animation:pulse 1.5s ease infinite; margin-bottom:8px; }
        }
      `}</style>

      <h1 style={{ fontFamily: "var(--font-oxanium)", fontSize: 22, fontWeight: 700, letterSpacing: "-0.3px", color: "#0F172A", margin: "0 0 6px" }}>Диалоги</h1>
      <p style={{ fontSize: 14, color: "#94A3B8", margin: "0 0 20px" }}>История переписки пациентов с Элей</p>
      ...
      {!error && (
        <div className="es-sessions-table-wrap">
          <div style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 10, overflow: "hidden", minWidth: 600, boxShadow: "0 1px 2px rgba(15,23,42,0.04), 0 4px 12px rgba(15,23,42,0.05)" }}>
              ...
              <tbody>
                {loading
                  ...
                  : sessions.map((session, idx) => (
                      ...
                      >
                        <td style={{ padding: "13px 16px", color: "#94A3B8", fontSize: 13, borderBottom: idx < sessions.length - 1 ? "1px solid #f5f5f5" : "none", whiteSpace: "nowrap" }}>
                          {formatDate(session.createdAt)}
                        </td>
                        <td style={{ padding: "13px 16px", color: "#0F172A", borderBottom: idx < sessions.length - 1 ? "1px solid #f5f5f5" : "none", maxWidth: 300, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {session.firstMessage ? (session.firstMessage.length > 60 ? session.firstMessage.slice(0, 60) + "…" : session.firstMessage) : "—"}
                        </td>
                        <td style={{ padding: "13px 16px", color: "#94A3B8", borderBottom: idx < sessions.length - 1 ? "1px solid #f5f5f5" : "none", textAlign: "center" }}>
