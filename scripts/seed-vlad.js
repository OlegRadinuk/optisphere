...
if (existing) {
  db.prepare(`
    UPDATE clients SET
      ...
      api_key = @api_key,
      base_url = @base_url,
      tg_token = COALESCE(NULLIF(@tg_token, ''), tg_token),
      tg_chat_id = COALESCE(NULLIF(@tg_chat_id, ''), tg_chat_id),
  ...
  `).run({
    ...
    tg_token: process.env.VLAD_TG_TOKEN || "",
    tg_chat_id: process.env.VLAD_TG_CHAT_ID || "1069707783",
    widget_color: "#1e3a5f",
    widget_title: "Влад",
